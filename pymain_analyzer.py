...
def analyze_speed_combined(url, timeout=120):
    "This funtion is used to analyze the page speed of the website using PageSpeed Insights API and web scraping."
...
class BackgroundRunner:
    ...
    def analyze_speed(self, url):
        return analyze_speed_combined(url)

    def get_ss(self, url, path):
        web.capture_screenshot(url, path)

    def get_ss_async(self, url, path):
        task_id = uuid.uuid4().hex
        self.executor.submit_stored(task_id, self.get_ss, url, path)
        return task_id

    def analyze_async(self, url):
        task_id = uuid.uuid4().hex  # Generate a unique task ID
        self.executor.submit_stored(task_id, self.analyze_speed, url)
        return task_id

    def task_status(self, task_id):
        if not self.executor.futures.done(task_id):
            return {"status":"running"}
        res = self.executor.futures.pop(task_id).result()
        
        return {"status": "completed","score":res[0], "paint":res[1],"time":res[2],"size":res[3],
                        "requests":res[4]}
...
def Cart_template(website_url):
    ...
    def initial():
        static_folder_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static')
        task_id_2 = background_runner.get_ss_async(url, os.path.join(static_folder_path, "Rectangle.png"))

        

    def check_title(title_to_execute):
        "heck if the title is none empty"
    ...
    def insert_image(url, image_name, yaxis=6.47, xaxis=1.52):
        try:
            ...
            # Open the template
            template = Image.open(os.path.join(static_folder_path, 'image.jpg'))
            ...
            template.save(os.path.join(static_folder_path, 'image.jpg'))
    ...
    def check_file(url):
        try:
            if not os.path.exists(os.path.join(static_folder_path, 'image.jpg')):
    ...
    for i in range (2):
        # try:
            ...
            if response.status_code == 200:
                ...
                l=[ f"Analyzed {website_url}:",email_advice,best_selling[0],best_selling[1],f"Theme Name is {boomer[0]}.",f"Theme Version is {boomer[1]}.",f"Furthermore Templates Present are {boomer[2]}.",website_url,website_url]
                return l
                # return l
            else:
...
def analyzing():
    ...
    website_url = "https://masterpiecejewellery.com.au/"
...
@app.route("/test1")
def test1():
    url = "https://amamaslullaby.com/"
    f=Cart_template(url)
    
    task_id = background_runner.analyze_async(url)
  

    ret_dic = {"title":f[0], "price":f[1], "task_id":task_id}
   
    return jsonify(ret_dic)

@app.route("/test2")
def test2():
    url = "https://masterpiecejewellery.com.au/"
    a = processing(url)
    ret_dic = {"website":a[0], "name":a[5], "version":a[6], "score":a[1], "templates":a[7], "klaviyo":a[2]}
    return jsonify(ret_dic)


@app.route("/analysis1", methods=['POST'])
def analysis1():
    url = request.get_json().get("website_url").strip()
    print(url)
    f=Cart_template(url)
    task_id = background_runner.analyze_async(url)
    ret_dic = {"title":f[0], "price":f[1], "task_id":task_id}
...
def is_shopify_website(url):
    try:
        ...
        # Check for common Shopify indicators in the HTML
        shopify_indicators = [
            'cdn.shopify.com',
            'shopify',
            'Shopify',
...
@app.route('/check_shopify', methods=['POST'])
def check_shopify():
    url = request.get_json().get("website_url").strip()
    if not url:
        return jsonify({"error": "URL parameter is required"}), 400
    
    is_shopify = is_shopify_website(url)
    return jsonify({"is_shopify": is_shopify})
